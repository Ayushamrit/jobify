...
const Login = () => {
    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            ...
            if (existingRole) {
                // Auto-switch the role toggle so user can retry immediately
                setInput(prev => ({ ...prev, role: existingRole }));
            }
            toast.error(serverMsg || error.message || "Google Sign-In failed");
        } finally {
            dispatch(setLoading(false));
        }
    }
